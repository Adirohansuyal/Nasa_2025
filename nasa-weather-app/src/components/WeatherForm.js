...
const WeatherForm = ({ onFetch, loading }) => {
  ...
  const handleCitySearch = async () => {
    ...
    try {
      ...
      setError("🛰️ Failed to fetch city coordinates from space.");
    }
  };
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn" disabled={loading} style={{width: '100%', fontSize: '1.1rem'}}>
          {loading ? '🛰️ Transmitting to NASA...' : '🚀 Launch Data Mission'}
        </button>
      </form>
    </div>
  );
